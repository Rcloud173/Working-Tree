...
const SkeletonJobCard = () => (
  <div className="bg-white rounded-2xl border border-gray-100 p-5 animate-pulse shadow-sm">
    ...
    <div className="flex gap-2 mt-3">
      ...
      <div className="h-9 bg-gray-100 rounded-xl w-12" />
    </div>
  </div>
);
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div>
            <h2 className="font-bold text-gray-900">Apply for Position</h2>
            <p className="text-sm text-gray-500 mt-0.5">{job.title} at {job.company}</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-xl text-gray-500 transition"><X size={18} /></button>
        </div>
        {done ? (
          <div className="p-10 text-center">
            <CheckCircle size={52} className="text-green-500 mx-auto mb-3" />
            <p className="font-bold text-gray-900 text-lg">Application Submitted!</p>
            <p className="text-sm text-gray-500 mt-1">We've sent your application to {job.company}</p>
          </div>
        ) : (
          <>
            <div className="p-5">
              <label className="text-sm font-bold text-gray-700 block mb-2">Cover Letter <span className="text-red-400">*</span></label>
              <textarea value={coverLetter} onChange={(e) => setCoverLetter(e.target.value)}
                placeholder="Tell them why you're a great fit for this role..."
                className="w-full h-36 p-4 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-200 resize-none transition" />
              <p className="text-xs text-gray-400 mt-1.5">{coverLetter.length}/500 characters</p>
              <div className="mt-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                <p className="text-xs text-gray-500">📎 Your profile details will be shared with this employer. You can also attach your resume from your profile settings.</p>
              </div>
            </div>
...
const JobCard = ({ job, onSelect, onApply, isSelected }) => {
  ...
  return (
    <div onClick={() => onSelect(job)}
      className={`bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all cursor-pointer ${isSelected ? 'border-green-400 ring-2 ring-green-100' : 'border-gray-100 hover:border-green-200'}`}>
      {job.featured && (
        <div className="px-4 pt-3 pb-0">
          <span className="text-xs font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full border border-orange-100">⭐ Featured</span>
        </div>
      )}
      <div className="p-5">
        <div className="flex items-start gap-3">
          <div className="w-12 h-12 bg-green-50 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0 border border-green-100">
            {job.companyLogo}
          </div>
          ...
          <button onClick={toggleSave} disabled={saving}
            className={`p-1.5 rounded-xl transition flex-shrink-0 ${saved ? 'text-green-600 bg-green-50' : 'text-gray-400 hover:bg-gray-100'}`}>
            {saving ? <Loader size={14} className="animate-spin" /> : saved ? <Bookmark size={14} fill="currentColor" /> : <BookmarkPlus size={14} />}
          </button>
        </div>

        <div className="flex flex-wrap gap-1.5 mt-3">
          <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${typeColors[job.type] || 'bg-gray-100 text-gray-600'}`}>{job.type}</span>
          <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-gray-100 text-gray-600">{job.category}</span>
          <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-green-50 text-green-700">{job.experience}</span>
        </div>

        <div className="flex items-center justify-between mt-3">
          <div>
            <p className="text-xs font-bold text-gray-800">{job.salary}</p>
...
const JobDetailPanel = ({ job, onApply, onClose }) => {
  if (!job) return (
    <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center shadow-sm h-64 flex flex-col items-center justify-center">
      <Briefcase size={40} className="text-gray-200 mb-3" />
      <p className="text-gray-400 font-medium text-sm">Select a job to see details</p>
    </div>
  );
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="p-5 border-b border-gray-100">
        <div className="flex items-start gap-4">
          <div className="w-16 h-16 bg-green-50 rounded-2xl flex items-center justify-center text-3xl border border-green-100">
            {job.companyLogo}
          </div>
          <div className="flex-1">
            <div className="flex items-start justify-between gap-2">
              <h2 className="font-black text-gray-900 text-lg leading-tight">{job.title}</h2>
              {job.featured && <span className="text-xs font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full border border-orange-100 flex-shrink-0">⭐ Featured</span>}
            </div>
            <p className="text-sm text-gray-600 font-semibold mt-0.5">{job.company}</p>
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          {[
            { label: 'Salary', value: job.salary, icon: <DollarSign size={13} className="text-green-600" /> },
            { label: 'Type', value: job.type, icon: <Briefcase size={13} className="text-green-600" /> },
            { label: 'Experience', value: job.experience, icon: <Star size={13} className="text-green-600" /> },
            { label: 'Deadline', value: job.deadline, icon: <Clock size={13} className="text-green-600" /> },
      ...
      <div className="p-5 space-y-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {job.skills.map(skill => (
              <span key={skill} className="px-3 py-1 bg-green-50 text-green-700 text-xs font-semibold rounded-full border border-green-100 flex items-center gap-1">
                <Tag size={9} />{skill}
              </span>
            ))}
          </div>
        ...
        <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
          ...
          <p className="text-xs text-gray-400">{job.applicationsCount} applications received</p>
        </div>
      </div>

      <div className="p-5 border-t border-gray-100">
        {job.applied ? (
          <div className="w-full py-3 bg-green-50 border border-green-200 text-green-700 rounded-xl text-sm font-bold flex items-center justify-center gap-2">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-xl font-black text-gray-900 flex items-center gap-2">
                <Briefcase size={22} className="text-green-600" /> Opportunities
              </h1>
              <p className="text-xs text-gray-500 mt-0.5">Agri jobs, consultancies & government roles</p>
            </div>
            <div className="flex gap-2">
              <span className="text-xs bg-green-50 text-green-700 px-2 py-1 rounded-lg font-bold border border-green-100">{filteredJobs.length} Jobs</span>
              <button onClick={() => setShowFilters(!showFilters)}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold border transition ${showFilters ? 'bg-green-50 text-green-700 border-green-200' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
            ...
            <div className="mt-3 flex flex-wrap gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
              <div>
                ...
                <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)}
                  className="text-sm border border-gray-200 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-green-200">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center shadow-sm">
                ...
                <p className="font-bold text-gray-900 text-lg">No applications yet</p>
                <p className="text-gray-400 text-sm mt-2">Start applying for opportunities!</p>
              ...
              applications.map(app => (
                <div key={app._id} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-green-50 rounded-2xl flex items-center justify-center text-2xl border border-green-100 flex-shrink-0">{app.logo}</div>
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-gray-900 text-sm truncate">{app.job.title}</p>
                      ...
                      <div className="flex items-center gap-1 mt-0.5">
                        ...
                        <span className="text-xs text-gray-400">Applied {app.appliedAt}</span>
                      </div>
                    </div>
                    <span className={`text-xs font-bold px-3 py-1.5 rounded-xl border flex-shrink-0 ${statusColors[app.status] || 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                      {app.status}
                    </span>
                  </div>
                </div>
